...
public class PlayerBehaviour : MonoBehaviour
{
    ...
    private void FixedUpdate(){
        ...
        // Check if we are running either in the Unity editor or in a standalone build.
            ...
            // If the mouse is held down (or the screen is pressed on the Mobile)
            if(Input.GetMouseButton(0)){
                horizontalSpeed = CalculateMovement(Input.mousePosition);
            }

        // Check if we are running on mobile device
        #elif UNITY_IOS || UNITY_ANDROID
            if(horizMovement == MobileHorizMovement.Accelerometer){
                // Move player based on direction of the accelrometer
    ...
    private float CalculateMovement(Vector3 pixelPos){
        ...
        if(worldPos.x < 0.5f){
            ...
            // Otherwise if we're on the left
            xMove = 1;
        }
